...
class CreditManager
{
    ...
    public function hasRefundForReference(string $reference_id): bool
    {
        ...
        $candidates = array_values(array_unique([
            'refund_' . $reference_id,
            $reference_id . '_refund',
        ]));

        foreach ($candidates as $candidate) {
            try {
                $docs = $this->db->collection('credit_transactions')
                    ->where('reference_id', '=', $candidate)
                    ->limit(1)
                    ->documents();
                foreach ($docs as $doc) {
                    if ($doc->exists() && ($doc->data()['type'] ?? '') === 'refund') {
                        return true;
                    }
                }
            } catch (\Throwable $e) {
                error_log('[CreditManager] Refund lookup failed: ' . $e->getMessage());
            }
        }
    ...
    /**
     * Idempotently refund SMS credits after a late provider failure webhook.
     *
     * @return array{refunded:bool,amount:int,reason?:string}
     */
    public function refundSmsOnProviderFailure(
        string $location_id,
        int $amount,
        ...
        if ($amount <= 0 || trim($location_id) === '') {
            return ['refunded' => false, 'amount' => 0, 'reason' => 'nothing_to_refund'];
        }

        if ($this->hasRefundForReference($billing_reference_id)) {
            return ['refunded' => false, 'amount' => 0, 'reason' => 'already_refunded'];
        }

        $refundRef = 'refund_' . $billing_reference_id;
        ...
        try {
            ...
            error_log('[CreditManager] Late SMS refund failed: ' . $e->getMessage());
            return ['refunded' => false, 'amount' => 0, 'reason' => $e->getMessage()];
        }
    }

    /**
     * Resolve the billing reference used when the original SMS debit was recorded.
     */
    public function resolveBillingReferenceId(array $messageData, string $fallbackReference = ''): string
    {
        foreach (['billing_reference_id', 'batch_id', 'message_id'] as $field) {
            $value = trim((string)($messageData[$field] ?? ''));
            if ($value !== '') {
                return $value;
            }
        }

        return trim($fallbackReference);
    }

    public function resolveAgencyIdForLocation(string $location_id): string
    {
        $rawLocId = preg_replace('/^ghl_/', '', trim($location_id));
        if ($rawLocId === '') {
            return '';
        }

        try {
            $snap = $this->db->collection('agency_subaccounts')->document($rawLocId)->snapshot();
            if ($snap->exists()) {
                return trim((string)($snap->data()['agency_id'] ?? ''));
            }
        } catch (\Throwable $e) {
            error_log('[CreditManager] Agency lookup failed: ' . $e->getMessage());
        }

        return '';
    }

    public function agencyMasterLockEnabled(string $agency_id): bool
    {
        ...
        return $this->get_agency_master_lock($agency_id);
    }
}
