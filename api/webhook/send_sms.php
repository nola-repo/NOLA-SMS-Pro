...
function clean_numbers($numberString): array
{
    ...
    $numbers = is_array($numberString)
        ? $numberString
        : preg_split('/[,;]/', $numberString);
...
/* DATA OBJECT (THIS IS YOUR CASE) */

if (!$number_input && isset($payload['data'])) {
...
if (!$number_input && isset($payload['contact'])) {
...
if (empty($validNumbers)) {
    ...
    echo json_encode([
        "status"=>"error",
        "message"=>"No valid PH numbers",
        "received"=>$number_input
    ]);
