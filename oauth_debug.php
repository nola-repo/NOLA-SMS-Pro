...
if ($code) {
    foreach ($apps as $key => $app) {
        if ($forceApp && $forceApp !== $key) continue;
...
body{font-family:'Inter',system-ui,sans-serif;background:#0f172a;color:#e2e8f0;min-height:100vh;padding:32px 16px}
.wrap{max-width:860px;margin:0 auto}
h1{font-size:24px;font-weight:800;letter-spacing:-0.5px;color:#f8fafc;margin-bottom:4px}
.badge-env{display:inline-block;background:#7c3aed;color:#fff;padding:2px 10px;border-radius:99px;font-size:11px;font-weight:700;margin-left:10px}
...
.card-title{font-size:15px;font-weight:700;color:#f1f5f9;margin-bottom:16px;display:flex;align-items:center;gap:10px}
.form-row{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px}
input[type=text]{flex:1;min-width:200px;background:#0f172a;border:1px solid #334155;border-radius:10px;padding:10px 14px;color:#f1f5f9;font-family:inherit;font-size:13px;outline:none}
input[type=text]:focus{border-color:#6366f1}
select{background:#0f172a;border:1px solid #334155;border-radius:10px;padding:10px 14px;color:#f1f5f9;font-size:13px}
button{background:#6366f1;color:#fff;border:none;border-radius:10px;padding:10px 24px;font-weight:700;font-size:13px;cursor:pointer}
button:hover{background:#4f46e5}
label{font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:0.05em;display:block;margin-bottom:6px}
.result-block{margin-top:20px}
.result-title{font-size:13px;font-weight:700;color:#94a3b8;margin-bottom:8px;display:flex;align-items:center;gap:8px}
.note{font-size:12px;color:#475569;margin-top:8px;line-height:1.5}
.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}
@media(max-width:600px){.grid{grid-template-columns:1fr}}
.kv{font-size:12px;margin-bottom:8px}
.kv strong{color:#7dd3fc;display:block;margin-bottom:2px}
.kv span{color:#e2e8f0;word-break:break-all}
.warn{background:#7c1d1d;border:1px solid #ef4444;color:#fca5a5;padding:12px;border-radius:10px;font-size:13px;margin-bottom:16px}
.ok{background:#14532d;border:1px solid #22c55e;color:#86efac;padding:12px;border-radius:10px;font-size:13px;margin-bottom:16px}
</style>
</head>
<body>
<div class="wrap">
    ...
    <div class="card">
        ...
        <form method="GET">
            ...
            <div style="margin-bottom:14px">
                ...
                <div class="form-row">
                    <input type="text" name="code" value="<?= htmlspecialchars($code) ?>" placeholder="Paste code=XXXXX value here" required>
                </div>
            </div>
            <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:14px">
                <div>
                    <label>Redirect URI (must match install link)</label>
                    <input type="text" name="redirect_uri" value="<?= htmlspecialchars($redirectUri) ?>" style="width:340px">
                </div>
                <div>
                    <label>Force App (optional)</label>
                    <select name="app">
                        <option value="" <?= !$forceApp ? 'selected' : '' ?>>Try Both (recommended)</option>
                        <option value="subaccount" <?= $forceApp==='subaccount'?'selected':'' ?>>Sub-account only</option>
...
<?php if (!$code): ?>
    <div class="card">
        ...
        ?>
        <a href="<?= $installUrl ?>" target="_blank" style="color:#6366f1;font-size:13px;word-break:break-all"><?= $installUrl ?></a></div>
        <ol start="2" style="font-size:13px;color:#94a3b8;line-height:2;padding-left:20px">
            ...
            <li><strong>Before the page finishes loading</strong>, copy the <code>code=XXXXX</code> value from your browser URL bar</li>
...
<?php if ($code && $results): ?>
    ...
    <div class="card">
        ...
        <div class="grid">
            ...
            <div>
                <p style="font-size:13px;font-weight:700;color:#f1f5f9;margin-bottom:8px"><?= htmlspecialchars($app['label']) ?></p>
                <div class="kv"><strong>client_id</strong><span><?= htmlspecialchars($app['client_id']) ?></span></div>
                <div class="kv"><strong>client_secret (tail)</strong><span>…<?= htmlspecialchars(substr($app['client_secret'], -8)) ?></span></div>
    ...
    <div class="card">
        ...
        <div class="result-block">
            ...
            <div class="grid">
                ...
                foreach ($show as $f):
                    if (!array_key_exists($f, $r['parsed'])) continue;
                ?>
                <div class="kv"><strong><?= $f ?></strong><span><?= htmlspecialchars((string)$r['parsed'][$f]) ?></span></div>
                <?php endforeach; ?>
                <div class="kv"><strong>access_token (head)</strong><span><?= htmlspecialchars(substr($r['parsed']['access_token'] ?? '', 0, 30)) ?>…</span></div>
            </div>
        </div>
        <?php endif; ?>
    </div>
    ...
    <div class="card">
        ...
        <p class="note">
            ✅ <code>client_id</code> should end in <strong>mmn30t4f</strong> (Sub-account app) for location tokens.<br>
            ❌ If it ends in <strong>mnqxvtt3</strong> the token was saved by the Agency app — refresh will use wrong credentials.
        </p>
        <?php else: ?>
        <div class="warn">⚠️ No Firestore document found for location <code><?= htmlspecialchars($locationId) ?></code> in <code>ghl_tokens</code>.</div>
        <?php endif; ?>
    </div>
    <?php endif; ?>
    ...
    <div class="card">
        ...
        if ($subOk):  ?>
            <div class="ok">✅ Sub-account app exchange succeeded — GHL credentials are correct. Re-install should now write a location token.</div>
        <?php elseif ($agOk): ?>
            <div class="warn">
        ...
        <?php elseif (!$subOk && !$agOk): ?>
            <div class="warn">
                ❌ Both exchanges failed. GHL error for Sub-account: <strong><?= htmlspecialchars($subErr) ?></strong><br><br>
                Common causes:<br>
        ...
        <?php endif; ?>
            <p style="margin-top:8px">
                ...
                <a href="https://marketplace.leadconnectorhq.com/" target="_blank" style="color:#6366f1">GHL Developer Portal</a>
