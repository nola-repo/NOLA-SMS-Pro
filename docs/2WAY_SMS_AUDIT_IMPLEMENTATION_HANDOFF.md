...
## Alternative Provider Research — Cast.ph

Date noted: 2026-08-20

Source: https://www.cast.ph/pricing

Flagged as a potential alternative or backup to UniSMS for 2-way SMS virtual number provisioning due to UniSMS setup cost concerns.

### Cast.ph One-Way SMS Pricing (Top-Up, No Subscription Fee)

| Plan | Top-Up | SMS Credits | Rate/SMS | Sender IDs | API |
| --- | --- | --- | --- | --- | --- |
| Free Trial | ₱0 | 5 SMS | Free | 0 | — |
| Starter | ₱875 | 2,500 SMS | ₱0.35 | 1 | ✓ API |
| Basic | ₱3,200 | 10,000 SMS | ₱0.32 | 1 | ✓ API |
| Standard | ₱9,000 | 30,000 SMS | ₱0.30 | 3 | ✓ API |
| Growth | ₱27,000 | 100,000 SMS | ₱0.27 | 5 | ✓ API |
| Business | ₱75,000 | 300,000 SMS | ₱0.25 | 10 | ✓ API + SMPP |
| Premium | ₱154,000 | 700,000 SMS | ₱0.22 | 20 | ✓ API + SMPP |
| Enterprise | ₱400,000 | 2,000,000 SMS | ₱0.20 | 20 | ✓ API + SMPP |

All plans include: Unlimited Contacts & Subscribers, Unlimited Team Members, Unlimited Campaign History, URL Shortener with Analytics, API, SMPP, Sender ID Registration.

### Cast.ph Two-Way SMS Options

- **Virtual SIM** — ₱1,500/month. Local PH mobile number, unlimited SMS (spam not allowed), bulk messages. *(Listed as hidden/coming soon on their pricing page as of 2026-08-20.)*
- **Access Code** — Talk to us (custom pricing). Toll-free incoming SMS, short codes, keyword registration.

### UniSMS vs Cast.ph Cost Comparison

| Item | UniSMS | Cast.ph |
| --- | --- | --- |
| Setup fee | ₱5,000 (one-time) | None confirmed for Virtual SIM |
| Monthly virtual number | ₱2,000/month | ₱1,500/month (Virtual SIM) |
| SMS credits | Separate top-up | Separate top-up (one-way plans) |
| 2-way support | ✓ Virtual numbers, API confirmed | ✓ Virtual SIM (needs API confirmation) |
| API documentation | Confirmed available | Needs review before integration |

### Notes and Open Items

- Cast.ph Virtual SIM at ₱1,500/month is cheaper than UniSMS at ₱2,000/month if the feature set and API are equivalent.
- Must confirm Cast.ph webhook/API shape for inbound SMS before treating this as a real alternative — the backend phases in this document are written against UniSMS payload contracts.
- If Cast.ph is adopted, Phase 0 provider contract confirmation, Phase 2 outbound method, Phase 3 inbound resolver, and UniSmsProvider all need a Cast.ph parallel implementation or an abstracted provider interface.
- Current plan remains UniSMS. Cast.ph is logged here for future evaluation or as a fallback if UniSMS virtual number provisioning stalls.

Contact for Cast.ph: inquirenow@cast.ph | +63 (998) 555-3473

---

## Client Context Note — Norwin Lacson (2026-08-20)

From a conversation with subaccount/client Norwin Lacson:

> **Norwin:** "Ano payung kulang sa 2way sms natin? Yung actual number nalang?"
>
> **Reply:** "Yes po sir, meron na po kaming planned sa 2-ways SMS for UniSMS."
>
> **Norwin:** "Yung number sa user side, bale request number din siya no? Parang sender ID?"
>
> **Reply:** "Yes po sir."

### Summary

- The only remaining blocker for 2-way SMS is the virtual number provisioning from UniSMS.
- All architecture, planning, and implementation phases are documented and in staging.
- The virtual number functions as a dedicated inbound/outbound number per location — analogous to a sender ID but mobile-number-based and capable of receiving replies.
- Once numbers are issued, Phase 0 contract confirmation can proceed immediately, followed by backend phases 1–3 as the critical path.
