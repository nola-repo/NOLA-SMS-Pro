...
The project uses `cloudbuild.yaml` at the repo root. From the project root, run:

```bash
gcloud builds submit --config=cloudbuild.yaml .
```

This will build the Docker image, push it to Container Registry, and deploy the `sms-api` service to Cloud Run (`asia-southeast1`).

---

### Step 2 — Verify the Cloud Run Service URL

The Cloud Run service URL is:

```
https://sms-api-116662437564.asia-southeast1.run.app
```

The cron endpoint is:

```
https://sms-api-116662437564.asia-southeast1.run.app/api/webhook/retrieve_status.php
```
...
>
> ```bash
> curl https://sms-api-116662437564.asia-southeast1.run.app/api/webhook/retrieve_status.php
> ```
>
...
| **Target type** | HTTP |
| **URL** | `https://sms-api-116662437564.asia-southeast1.run.app/api/webhook/retrieve_status.php` |
| **HTTP Method** | GET |
