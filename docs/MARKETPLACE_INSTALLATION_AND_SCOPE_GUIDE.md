...
**Status:** implementation-aligned guide
...
app is a **Sub-account target-user app** and eight Agency-only scopes are
unavailable for this target-user type. After the 2026-07-07 cleanup, the
custom-value scopes were removed from the intended live set.

The 10 scopes that should remain selected are:
...
This is the desired least-privilege set for the current runtime. The removed
custom-value scopes do not have a corresponding runtime API call in the
current code. The remaining ten selected scopes map to messaging, contact,
location, notification, and agency-to-location installation behavior.
...
## 2. Important installation defect corrected

Before the 2026-07-07 fix, agency provisioning called:
...
For designated sub-account access, provisioning now uses:

`GET /oauth/installedLocations`

with `oauth.readonly`, then exchanges only those installed location IDs through `POST /oauth/locationToken` with `oauth.write`. Continue listening for `INSTALL` events so future location installations can be provisioned individually. This protects agency/bulk install from silently authorizing all agency locations in NOLA.

Relevant current code:

- `api/agency/install_provision.php` — fetches installed locations only for provisioning.
...
The previous OAuth-consent mismatch in the hard-coded reinstall/debug links in
`ghl_callback.php` and `oauth_debug.php` has also been corrected. Those links
now request the same lean scope set and no longer request workflow or location
tag catalog permissions.
...
| `locations/customValues.readonly` | Off | Leave off | No current NOLA runtime path reads HighLevel location custom values. |
| `locations/customValues.write` | Off | Leave off | No current NOLA runtime path creates, updates, or deletes HighLevel location custom values. |
...
| Support Ticket Submission | Ticket is stored and returned to the user, an internal admin notification is written, and a central GHL support-ticket alert tag is cycled. | Implemented. Requires `NOLA_ALERT_SUPPORT_TICKET_TAG` or the default `nola-support-ticket-alert` to match the published workflow. |
...
For Low Balance, Sender ID, Top Up Success, Welcome Onboarding, and Support Ticket Submission to work reliably:
...
2. Call `NotificationService::notifySupportTicketSubmitted`.
3. Upsert the account owner in the central location, write alert type, ticket details, source-location fields, and admin notes, then cycle a dedicated tag such as `nola-support-ticket-alert`.
4. Treat notification failure as non-fatal to ticket creation, but surface it in operational logs.
...
12. Support ticket creation produces one internal notification and one configured workflow alert.
...
1. Run disposable-location regression tests for installed-location provisioning (`oauth.readonly`).
2. Confirm the live marketplace scope screen still matches the canonical 10-scope set.
3. Keep the two unused custom-value scopes off unless a future feature explicitly needs them.
4. Decide explicitly between Agency-only installation now and future dual installer support.
5. Verify the support-ticket workflow tag and custom fields in the central notification location.
